...
public class MessagingEndpoint {
     ...
     */
    public void sendMessage(@Named("message") String message, @Named("to") ArrayList<String> to) throws IOException {
        if (legalMessage(message)) {
            Sender sender = new Sender(API_KEY);
            Message msg = new Message.Builder().addData("message", message).build();
            for(String toID : to) {
                Result result = sender.send(msg, toID, 5);
            }
        }
    }
